  ...
  switch ($_GET['act']) {
    ...
    case 'checkout': {
        ...
        if (isset($_POST['total_prices']) && $_POST['total_prices'] != "") {
          ...
          $invoice_id = "XFASHION" . rand(0, 999999);
...
<script>
  // Tạo bông tuyết với delay ngẫu nhiên
  function createSnowflake() {
      const snowflake = document.createElement('div');
      snowflake.classList.add('snowflake');
      snowflake.style.left = Math.random() * 100 + 'vw'; // Vị trí ngang ngẫu nhiên
      snowflake.style.animationDuration = Math.random() * 5 + 5 + 's'; // Tốc độ rơi khác nhau
      snowflake.style.opacity = Math.random(); // Độ mờ khác nhau
      snowflake.style.width = snowflake.style.height = Math.random() * 8 + 2 + 'px'; // Kích thước ngẫu nhiên
      snowflake.style.animationDelay = Math.random() * 5 + 's'; // Thêm khoảng trễ
  ...
  setInterval(createSnowflake, 1); // Tạo bông tuyết mỗi 1ms
</script>
